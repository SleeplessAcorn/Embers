...
public class ModifierSuperheater extends ModifierBase {
        ...
        if (event.getHarvester() instanceof EntityPlayer) {
            if (!event.getHarvester().getHeldItem(EnumHand.MAIN_HAND).isEmpty()) {
                ...
                if (ItemModUtil.hasHeat(s)) {
                    if (ItemModUtil.getModifierLevel(s, ItemModUtil.modifierRegistry.get(RegistrarEmbersItems.SUPERHEATER).name) > 0 && EmberInventoryUtil.getEmberTotal(event.getHarvester()) >= cost) {
    ...
    public void onHit(LivingHurtEvent event) {
        if (event.getSource().getTrueSource() instanceof EntityPlayer) {
            ...
            if (!s.isEmpty()) {
                if (ItemModUtil.hasHeat(s)) {
                    int superheatLevel = ItemModUtil.getModifierLevel(s, ItemModUtil.modifierRegistry.get(RegistrarEmbersItems.SUPERHEATER).name);
