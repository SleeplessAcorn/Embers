...
public class ItemClockworkPickaxe extends ItemTool implements IModeledItem, IEmberChargedTool {
    ...
    public ItemClockworkPickaxe(String name, boolean addToTab) {
        super(materialClockworkPickaxe, Sets.newHashSet(Blocks.STONE));
    ...
    public boolean canHarvestBlock(IBlockState state, ItemStack stack) {
        return !(state.getBlock().getHarvestLevel(state) > 3 || state.getBlock().getHarvestTool(state) != null && !state.getBlock().getHarvestTool(state).equals("pickaxe") && !state.getBlock().getHarvestTool(state).equals("shovel") && state.getBlock().getHarvestLevel(state) > -1 && !state.getMaterial().isToolNotRequired());
    }
