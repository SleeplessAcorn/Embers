...
public class BlockCaminiteLever extends BlockBase {
    ...
    public int getStrongPower(IBlockState blockState, IBlockAccess blockAccess, BlockPos pos, EnumFacing side) {
        return !blockState.getValue(POWERED).booleanValue() ? 0
                : (blockState.getValue(FACING).getFacing() == side ? 15 : 0);
    }
    ...
    public IBlockState withMirror(IBlockState state, Mirror mirrorIn) {
        return state
                .withRotation(mirrorIn.toRotation(state.getValue(FACING).getFacing()));
    }
